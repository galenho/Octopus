...
bool HTTPAccessor_impl::Run()
{
	...
	{	
		while (query_list_.pop(query))
		{
			...
			int32 use_time = (int32)(getMSTime() - begin_time);
...
int HTTPAccessor_impl::processFunc( void* ptr, int size, int nmemb, void *usrptr )
{
	HTTPAccessor_impl* p =(HTTPAccessor_impl*) usrptr;
	return p->Process( ptr, size, nmemb );
}

int HTTPAccessor_impl::Process( void* data, int size, int nmemb )
{
