...
bool UDPServer_impl::SendMsg( int32 conn_idx, const void* msg, uint32 len )
{
	return SocketMgr::get_instance()->SendMsg(conn_idx, msg, len);
}

void UDPServer_impl::Disconnect( int32 conn_idx )
{
	return SocketMgr::get_instance()->Disconnect(conn_idx);
}

char* UDPServer_impl::GetIpAddress( int32 conn_idx )
{
