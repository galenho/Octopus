...
void MongoConnection::set_conn_idx(int32 conn_idx)
{
	conn_idx_ = conn_idx;
}
...
{
	int size = query_list_.push_back(task);
	if (size == 1)
	{
		WakeUp();
	}
...
void MongoConnection::ProcessUpdateMany(UpdateManyMongoTask* query)
{
	...
	bson_t reply = BSON_INITIALIZER;
	mongoc_bulk_operation_t* bulk_update = mongoc_collection_create_bulk_operation(collection, false, NULL);
	mongoc_bulk_operation_update(bulk_update, query->filter_bson, query->update_bson, false);
	bool ret = mongoc_bulk_operation_execute(bulk_update, &reply, &error);
	bson_destroy(&reply);
	mongoc_bulk_operation_destroy(bulk_update);
...
void MongoConnection::ProcessDeleteOne(DeleteOneMongoTask* query)
{
	...
	bson_error_t error;
	bson_t reply = BSON_INITIALIZER;
	mongoc_bulk_operation_t* bulk_delete = mongoc_collection_create_bulk_operation(collection, true, NULL);
	mongoc_bulk_operation_remove_one(bulk_delete, query->doc_bson);
	bool ret = mongoc_bulk_operation_execute(bulk_delete, &reply, &error);
	bson_destroy(&reply);
	mongoc_bulk_operation_destroy(bulk_delete);
...
void MongoConnection::ProcessDeleteMany(DeleteManyMongoTask* query)
{
	...
	for (uint32 n = 0; n < query->doc_bson_list.size(); n++)
	{
		mongoc_bulk_operation_remove(bulk_delete, query->doc_bson_list[n]);
	}
	bool ret = mongoc_bulk_operation_execute(bulk_delete, &reply, &error);
	bson_destroy(&reply);
	mongoc_bulk_operation_destroy(bulk_delete);
