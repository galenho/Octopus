...
namespace SocketOps
{
	...
	{
		return (setsockopt(fd, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, NULL, 0) == 0);
	}

	// Set update accept context.
	bool SetUpdateAcceptContext(SOCKET connFd, SOCKET listenFd)
	{
		return (setsockopt(connFd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char*)&listenFd, sizeof(listenFd)) == 0);
	}
	...
	void CloseSocket(SOCKET fd)
	{
		/*
			...
			l_onoff 为非0，l_linger为非0，当套接口关闭时内核将拖延一段时间（由l_linger决定）。如果套接口缓冲区中仍残留数据，进程将处于睡眠状态，直 到（a）所有数据发送完且被对方确认，
					之后进行正常的终止序列（描述字访问计数为0）或（b）延迟时间到。此种情况下，应用程序检查close的返回值是非常重要的，如果在数据发送完并被确认前时间到，close将返回
					EWOULDBLOCK错误且套接口发送缓冲区中的任何数据都丢失。close的成功返回仅告诉我们发送的数据（和FIN）已由对方TCP确认，它并不能告诉我们对方应用进程是否已读了数据。
		...
		*/
		...
		int retv = closesocket(fd);
		//PRINTF_ERROR("closesocket fd = %d", fd);

		if (retv == SOCKET_ERROR)
		{
			printf("closesocket err = %d", WSAGetLastError());
		}
	}
