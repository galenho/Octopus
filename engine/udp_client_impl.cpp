...
void UDPClient_impl::Disconnect( int32 conn_idx )
{
	SocketMgr::get_instance()->Disconnect(conn_idx);
}

bool UDPClient_impl::SendMsg(int32 conn_idx, const void* msg, uint32 len)
{
	return SocketMgr::get_instance()->SendMsg(conn_idx, msg, len);
}

char* UDPClient_impl::GetIpAddress( int32 conn_idx )
{
