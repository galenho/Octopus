...
bool MongoDatabase_impl::Initialize(const string& uri, const string& db_name, int32 conn_count)
{
...
void MongoDatabase_impl::add_connect_ref( int32 conn_idx )
{
...
void MongoDatabase_impl::remove_connect_ref( int32 conn_idx )
{
	ASSERT(conn_idx < (int)connect_lists.size());
...
void MongoDatabase_impl::PushTask(MongoTask* task, int32 conn_idx)
{
	...
	{
		if (conn_idx >= (int)connect_lists.size())
		{
			return;
		}
