...
bool MySqlConnection::Init( const string& hostname,
						  uint16 port, 
...
void MySqlConnection::set_conn_idx(int32 conn_idx)
{
	conn_idx_ = conn_idx;
}
...
uint32 MySqlConnection::get_conn_idx()
{
	return conn_idx_;
}
...
void MySqlConnection::AddQuery(Statement* stmt, HandleInfo handler, DBTransactionFlag flag)
{
	// 如果连接的服务已经停止, 就不处理了
	if (!is_running_)
	{
	...
	query->stmt = stmt;
	int size = query_list_.push_back(query);
	if (size == 1)
	{
		WakeUp();
	}
...
void MySqlConnection::InsertFristQuery(Statement* stmt, HandleInfo handler)
{
	// 如果连接的服务已经停止, 就不处理了
	if (!is_running_)
	{
	...
	query->handler = handler;
	query->stmt = stmt;
	int size = query_list_.push_front(query);
	if (size == 1)
	{
		WakeUp();
	}
