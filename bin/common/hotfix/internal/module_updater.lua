...
local function check_updated(new_obj, old_obj, name, deep)
    ...
    if new_obj == old_obj then
        M.log_debug(string.format("%sUpdate %s: %s ---- Same", deep, name, signature))
        return true
    end
    if updated_sig[signature] then
        M.log_debug(string.format("%sUpdate %s: %s ---- Already updated", deep, name, signature))
        return true
    end
...
local function log_dbg(deep, from, to)
    M.log_debug(string.format("%ssetupvalue: (%s) -> (%s)", deep, tostring(from), tostring(to)))
end
