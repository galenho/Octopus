...
int lua_mongo_initialize(lua_State* L)
{
	...
	int32 conn_count = (int32)lua_tointeger(L, 4);
...
int lua_mongo_add_connect_ref(lua_State* L)
{
	...
	mongo_database_t* t = (mongo_database_t*)luaL_checkudata(L, 1, "mongo");
	uint32 conn_idx = (int32)lua_tointeger(L, 2);
...
int lua_mongo_command_simple(lua_State* L)
{
	...
	if (count >= 5)
	{
		conn_idx = (int32)lua_tointeger(L, 5);
	}
...
{
	// sample ---- db:create_index("player", {pid=1}, fun_callback)
...
int lua_mongo_find_one(lua_State* L)
{
	...
	if (count >= 7)
	{
		conn_idx = (int32)lua_tointeger(L, 7);
	}
...
int lua_mongo_find(lua_State* L)
{
	...
	if (count >= 8)
	{
		limit_count = (int32)lua_tointeger(L, 8);
	}
...
int lua_mongo_insert_one(lua_State* L)
{
	...
	if (count >= 6)
	{
		conn_idx = (int32)lua_tointeger(L, 6);
	}
...
int lua_mongo_insert_batch(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 4)
	{
		// sample ---- db:insert_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback)
		check_param(L, 4, "ustf");
	}
	else if (count == 5)
	{
		// sample ---- db:insert_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2})
		check_param(L, 5, "ustft");
	}
	else
	{
		// sample ---- db:insert_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 6, "ustftn");
	}
	...
	//stack_dump(L);
	InsertBatchMongoTask* query = new InsertBatchMongoTask();
	query->task_type = MONGO_TASK_TYPE_INSERT_BATCH;
	query->table_name = table_name;
	query->handler = handler;
	...
	for (; lua_next(L, documents_index) != 0; num_elements++, lua_index++)
	{
		...
		if (throw_error)
		{
			luaL_error(L, "lua_mongo_insert_batch ---- table param is error!");
		}
...
int lua_mongo_delete_one(lua_State* L)
{
	...
	if (count == 4)
	{
		// sample ---- db:delete_one("player", {obj_idx=1}, fun_callback)
		check_param(L, 4, "ustf");
	}
	else if (count == 5)
	{
		// sample ---- db:delete_one("player", {obj_idx=1}, fun_callback, {a=1, b=2})
		check_param(L, 5, "ustft");
	}
	else
	{
		// sample ---- db:delete_one("player", {obj_idx=1}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 6, "ustftn");
	}
...
int lua_mongo_delete_many(lua_State* L)
{
	...
	if (count == 4)
	{
		// sample ---- db:delete_many("player", {obj_idx=1}, fun_callback)
		check_param(L, 4, "ustf");
	}
	else if (count == 5)
	{
		// sample ---- db:delete_many("player", {obj_idx=1}, fun_callback, {a=1, b=2})
		check_param(L, 5, "ustft");
	}
	else
	{
		// sample ---- db:delete_many("player", {obj_idx=1}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 6, "ustftn");
	}
...
int lua_mongo_delete_batch(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 4)
	{
		// sample ---- db:delete_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback)
		check_param(L, 4, "ustf");
	}
	else if (count == 5)
	{
		// sample ---- db:delete_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2})
		check_param(L, 5, "ustft");
	}
	else
	{
		// sample ---- db:delete_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 6, "ustftn");
	}
	...
	//stack_dump(L);
	DeleteBatchMongoTask* query = new DeleteBatchMongoTask();
	query->task_type = MONGO_TASK_TYPE_DELETE_BATCH;
	query->table_name = table_name;
	query->handler = handler;
...
int lua_mongo_update_one(lua_State* L)
{
	...
	if (count == 5)
	{
		// sample ---- db:update_one("player", {obj_idx=1}, {["$set"] = {hp = 388}}, fun_callback)
		check_param(L, 5, "usttf");
	}
	else if (count == 6)
	{
		// sample ---- db:update_one("player", {obj_idx=1}, {["$set"] = {hp = 388}}, fun_callback, {a=1, b=2})
		check_param(L, 6, "usttft");
	}
	else
	{
		// sample ---- db:update_one("player", {obj_idx=1}, {["$set"] = {hp = 388}}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 7, "usttftn");
	}
...
int lua_mongo_update_many(lua_State* L)
{
	...
	if (count == 5)
	{
		// sample ---- db:update_many("player", {hp = 100}, {["$set"] = {hp = 200}}, fun_callback)
		check_param(L, 5, "usttf");
	}
	else if (count == 6)
	{
		// sample ---- db:update_many("player", {hp = 100}, {["$set"] = {hp = 200}}, fun_callback, {a=1, b=2})
		check_param(L, 6, "usttft");
	}
	else
	{
		// sample ---- db:update_many("player", {hp = 100}, {["$set"] = {hp = 200}}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 7, "usttftn");
	}
...
int lua_mongo_update_batch(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 4)
	{
		// sample ---- db:update_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback)
		check_param(L, 4, "ustf");
	}
	else if (count == 5)
	{
		// sample ---- db:update_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2})
		check_param(L, 5, "ustft");
	}
	else
	{
		// sample ---- db:update_batch("player", {{born_server="gs10001"}, {born_server="gs10002"}}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 6, "ustftn");
	}
	...
	stack_dump(L);
	UpdateBatchMongoTask* query = new UpdateBatchMongoTask();
	query->task_type = MONGO_TASK_TYPE_UPDATE_BATCH;
	query->table_name = table_name;
	query->handler = handler;
	...
	// 第3个参数
	//------------------------------------------
	/*  (1)将表压入栈顶：确保要遍历的表位于栈顶。
		(2)初始键压入 nil：调用 lua_pushnil(L) 开始遍历。
		(3)循环调用 lua_next：
			每次调用 lua_next(L, -2)（参数为栈索引，通常为表的位置）。
			它会弹出当前键，并压入下一个键值对（键和值）。
			若没有更多元素，返回 0。
		(4)处理键值对：在循环内读取键和值（栈顶为值，次顶为键）。
		(5)移除值保留键（可选）：根据需要调整栈，但通常直接保留键供下一次迭代。
	*/
	lua_pushnil(L);
	//stack_dump(L);
	...
	int lua_index = 1;
	for (; lua_next(L, documents_index) != 0; lua_index++)
	{
		//stack_dump(L);
		throw_error = !(lua_isnumber(L, -2)) || lua_tonumber(L, -2) != lua_index;
		if (throw_error)
		{
			luaL_error(L, "lua_mongo_update_batch ---- table param is error!");
		}
		
		UpdateBsonPair* update_pair = new UpdateBsonPair();

		lua_pushnil(L); // 遍历子表
		//stack_dump(L);
		for (; lua_next(L, documents_index) != 0; )
		{
			//stack_dump(L);
			throw_error = !(lua_isstring(L, -2));
			if (throw_error)
			{
				luaL_error(L, "lua_mongo_update_batch ---- table param is error!");
			}
			...
			if (strcmp("filter", member) == 0)
			{
				...
				update_pair->filter_bson = doc_bson;
			}
			else if (strcmp("update", member) == 0)
			{
				struct bson b;
				bson_create(&b);
				pack_dict(L, &b, false, 0);
				...
				update_pair->update_bson = doc_bson;
			}
			else
			{
				assert(false);
		...
		query->update_pair_list.push_back(update_pair);
...
int lua_mongo_update_one_upsert(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 5)
	{
		// sample ---- db:update_one("player", {born_server="gs10001"}, {born_server="gs10001"}, fun_callback)
		check_param(L, 5, "usttf");
	}
	else if (count == 6)
	{
		// sample ---- db:update_one("player", {born_server="gs10001"}, {born_server="gs10001"}, fun_callback, {a=1, b=2})
		check_param(L, 6, "usttft");
	}
	else
	{
		// sample ---- db:update_one("player", {born_server="gs10001"}, {born_server="gs10001"}, fun_callback, {a=1, b=2}, 0)
		check_param(L, 7, "usttftn");
	}
