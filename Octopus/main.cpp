...
#include "lua_lpeglabel/lpltree.h"
...
std::string g_server_path; // 服务器路径
...
static const luaL_Reg lua_reg_libs[] = {
	...
	{ "websocketclient",luaopen_lua_websocketclient},
...
int main(int argc, char* argv[])
{
	...
	if (argc >= 2)
	{
		g_server_path = argv[1];
	}

	if (argc >= 3)
	{
		g_log_file = g_server_path + "/" + argv[2];
	}

	if (argc >= 4)
	{
		g_config_file = g_server_path + "/" + argv[3];
	}
