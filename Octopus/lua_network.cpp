...
int lua_tcpserver_start(lua_State* L)
{
	...
	uint32 send_buffer_size = (uint32)lua_tointeger(L, 7);
	uint32 read_buffer_size = (uint32)lua_tointeger(L, 8);
	...
	bool ret = t->server->Start(ip, port, onconnected_handler, onclose_handler, onrecv_handler, send_buffer_size, read_buffer_size);
...
int lua_tcpclient_connect(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 8)
	{
	...
	t->client->Connect(ip, port, onconnected_handler, onclose_handler, onrecv_handler, send_buffer_size, read_buffer_size);
...
int lua_udpclient_connect(lua_State* L)
{
	...
	uint16 local_port = 0;
	t->client->Connect(ip, port, local_port, onconnected_handler, onclose_handler, onrecv_handler, send_buffer_size, read_buffer_size);
...
int lua_websocketclient_new(lua_State* L)
{
	websocketclient_t* t = (websocketclient_t*)lua_newuserdata(L, sizeof(*t));
	t->client = new WebSocketClient();
...
int lua_websocketclient_connect(lua_State* L)
{
	int count = lua_gettop(L);
	if (count == 8)
	{
	...
	websocketclient_t* t = (websocketclient_t*)luaL_checkudata(L, 1, "websocketclient");
	const char* ip = lua_tostring(L, 2);
	uint16 port = (uint16)lua_tointeger(L, 3);
...
int lua_websocketclient_sendmsg(lua_State* L)
{
	check_param(L, 4, "unsn");
...
int lua_websocketclient_disconnect(lua_State* L)
{
	check_param(L, 2, "un");
...
int lua_websocketclient_getipaddress(lua_State* L)
{
	check_param(L, 2, "un");
...
int lua_websocketclient_destroy(lua_State* L)
{
	check_param(L, 1, "u");
...
int luaopen_lua_websocketclient(lua_State* L)
{
	luaL_newmetatable(L, "websocketclient");
	lua_pushvalue(L, -1);
	lua_setfield(L, -2, "__index");

	luaL_setfuncs(L, lua_websocketclient_methods, 0);
	luaL_newlib(L, lua_websocketclient_functions);
