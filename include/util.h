...
//----------------------------------------------------------------------------------------
void AddPoint(void* p, const char* file, int line);
void DelPoint(void* p);
void PrintPointsInfo();

#define SAFE_NEWPOINT(P, T) { P=NULL; P=new T; AddPoint(P, __FILE__, __LINE__); }

#define SAFE_DELETE(P)       { if(P) { DelPoint(P);	delete P;   P=NULL;} }
#define SAFE_DELETE_ARRAY(P) { if(P) { DelPoint(P);	delete[] P; P=NULL;} }
